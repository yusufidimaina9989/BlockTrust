...
export class Market extends SmartContract {
  ...
  @prop(true)
  items: FixedArray<Item, typeof Market.ITEM_SLOTS>;
  ...
  public listItem(item: Item, itemIdx: bigint) {
    ...
    assert(
      !item.isEmptySlot,
      'new item cannot have the "isEmptySlot" flag set to true'
    );
    assert(item.price > 0n, "item price must be at least one satoshi");
    assert(
      !item.hasRequestingBuyer,
      "new item cannot have requesting buyer flag set to true"
    );
  ...
  public confirmBuy(itemIdx: bigint) {
    ...
    this.items[Number(itemIdx)].requestingBuyer = PubKeyHash(
      toByteString("0000000000000000000000000000000000000000")
    );
  ...
  public cancelBuy(itemIdx: bigint, buyerPubKey: PubKey, buyerSig: Sig) {
    ...
    assert(
      hash160(buyerPubKey) == item.requestingBuyer,
      "buyer invalid pubkey"
    );
    assert(this.checkSig(buyerSig, buyerPubKey), "buyer sig invalid");

    this.items[Number(itemIdx)].hasRequestingBuyer = false;
    this.items[Number(itemIdx)].requestingBuyer = Addr(
      toByteString("0000000000000000000000000000000000000000")
    );
